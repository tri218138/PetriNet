...
{
    clear_selection();//xóa object đang chọn trước đó
    switch (object.type) {
        case "circle":
...
function type_selected(text)// Hiển thị tên object trên thanh chọn
{
    var selected_label = document.getElementById("selected_label");
    if (selected_label != null)
        selected_label.innerHTML = text;
}
function clear_selection()// xóa hiển thị object đang chọn
{
    if (Selected != null)
    {
        Selected.remove();
...
function AddPlace(key, x, y, tokens)
{
    ...
    var place = paper.circle(x, y, PL_RADIUS).attr(place_attr);//object
    place.drag(drag_move, drag_start, drag_end);//di chuyển object
    place.click(place_click);//chọn object
...
function arrow_arc(x1, y1, x2, y2, dd)                  //��������� ���� ���������� �������
{
...
function draw_arc(x1, y1, x2, y2, isDouble)       // ������ ����
{
    if (isDouble == true) {
        y1 += DOUBLE_SHIFT;
        y2 += DOUBLE_SHIFT;
...
// redraw nghĩa là vẽ lại đúng số lượng token trong việc di chuyển (drag)
// thêm token: xóa hết tokens hiện tại -> push -> vẽ tokens
